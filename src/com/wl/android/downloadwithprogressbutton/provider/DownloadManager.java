...
package com.wl.android.downloadwithprogressbutton.provider;
...
public class DownloadManager {
	...
	ContentValues toContentValues(String packageName) {
	    ...
	    putIfNonNull(values, Downloads.COLUMN_NOTIFICATION_EXTRAS, mNotiExtras);
	    // trinea END
	...
	Cursor runQuery(ContentResolver resolver, String[] projection,
		...
		if ((mStatusFlags & STATUS_RUNNING) != 0) {
		    ...
		    // trinea BEGIN
		    parts.add(statusClause("=", Downloads.STATUS_INSUFFICIENT_SPACE_ERROR));
		    // trinea END
		}
    ...
    public String getLoacalUriById(long downLoadId){
    	 DownloadManager.Query query = new DownloadManager.Query().setFilterById(downLoadId);
    	 String result = null;
    	Cursor cursor = null;
        try {
    ...
    public String getDownLoadPath(long downloadId){
	    ...
	    String localUri = getLoacalUriById(downloadId);
    ...
    public  boolean isDownLoading(long downloadId ){		
		int downloadManagerStatus = getStatusById(downloadId);
		return downloadManagerStatus == DownloadManager.STATUS_RUNNING;
		
	}
	
	public  boolean isPending(long downloadId){
		int downloadManagerStatus = getStatusById(downloadId);
		return downloadManagerStatus == DownloadManager.STATUS_PENDING;
	}
	
	public  boolean isPaused(long downloadId ){
		int downloadManagerStatus = getStatusById(downloadId);
		return downloadManagerStatus == DownloadManager.STATUS_PAUSED;
		
	}
	
	public  boolean isFailed(long downloadId ){
		int downloadManagerStatus = getStatusById(downloadId);
		return downloadManagerStatus == DownloadManager.STATUS_FAILED;
		
	}
	/**
	 * 此方法描述的是： 根据当前任务数据库文件是否存在，判断是否下载成功
	 * 
	 * @return return boolean
	 * @author : wangliu94@163.com
	 * @version : 2014-8-9 下午04:19:50
	 */

	public boolean isDownLoaded(long downLoadId) {
		try {
			String localPath = getDownLoadPath(downLoadId);
	...
	public String getTitleById(long downloadId){
   	 DownloadManager.Query query = new DownloadManager.Query().setFilterById(downloadId);
	 String result = null;
	Cursor cursor = null;
    try {
        cursor = query(query);
        if (cursor != null && cursor.moveToFirst()) {
            result = cursor.getString(cursor.getColumnIndex(DownloadManager.COLUMN_TITLE));
        }
    } finally {
        if (cursor != null) {
            cursor.close();
	...
	public int getDownLoadPencent(long downloadId){
		int[] bytesAndStatus = getBytesAndStatus(downloadId);
		long progress = bytesAndStatus[0];
		long max = bytesAndStatus[1];
		int rate = 0;
		if (progress <= 0 || max <= 0) {
			rate = 0;
